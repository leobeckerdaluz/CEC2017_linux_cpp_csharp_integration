// CEC2017.cpp : Defines the exported functions for the DLL application.
//
...
#define FUNCTIONS_INPUT_PATH "/home/lbluz/git/GEO_OptimizationAlgorithms/src/CEC2017_linux_cpp_csharp_integration/cpp/"
...
extern "C" {
	// __declspec(dllexport) void cec17_test_func(double *x, double *f, int nx, int mx, int func_num)
	...
	void cec17_test_func(double *x, double *f, int nx, int mx, int func_num)
	{
		...
		if (ini_flag == 0)
		{
			...
			/* Load Matrix M*/
			sprintf(FileName, "%s/input_data/M_%d_D%d.txt", FUNCTIONS_INPUT_PATH, func_num, nx);
			...
			/* Load shift_data */
			sprintf(FileName, "%s/input_data/shift_data_%d.txt", FUNCTIONS_INPUT_PATH, func_num);
			fpt = fopen(FileName, "r");
			if (fpt == NULL)
			{
				std::cout << "\n Error: Cannot open input file for reading \n";
			...
			{
				sprintf(FileName, "%s/input_data/shuffle_data_%d_D%d.txt", FUNCTIONS_INPUT_PATH, func_num, nx);
				fpt = fopen(FileName, "r");
				if (fpt == NULL)
				{
					std::cout << "\n Error: Cannot open input file for reading \n";
